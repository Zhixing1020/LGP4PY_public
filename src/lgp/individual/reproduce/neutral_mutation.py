...
class LGPNeutralMutationPipeline(MutationPipeline):
    """
    this mutation only produces introns
    """
    P_STEP = "step"  # the number of free mutation step size
    P_INSERT = "prob_insert"
    P_DELETE = "prob_delete"
    NEUTRALMUT = "neutralmut"
    ...
    def setup(self, state:EvolutionState, base:Parameter):
        ...
        def_ = LGPDefaults.base.push(self.NEUTRALMUT)
    ...
    def produce_individual(self, subpopulation, ind, state, thread, no_clone:bool=False)->LGPIndividual:
        ...
        for _ in range(int(pickNum)):
            ...
            # Delete instruction
                ...
                if j.getTree(t).status:
                    continue  # skip effective instruction
                j.removeTree(t)
                j.evaluated = False
    ...
    def getLegalInsertIndex(self, ind:LGPIndividual, state:EvolutionState, thread):
        """get the instruction positions where effective registers < all possible registers"""
        res = ind.getConditionIndex(state, thread, lambda x:len(x.effRegisters)<ind.getNumRegs())
        return res
    
    def getLegalDeleteIndex(self, ind:LGPIndividual, state:EvolutionState, thread):
        """get the ineffective instructions, to be removed"""
        res = ind.getConditionIndex(state, thread, lambda x: not x.status)
        return res
    
    def check_points(self, p1:GPNode, p2:GPNode, state:EvolutionState, thread:int, ind:LGPIndividual, treeStr:GPTreeStruct):
        """make sure the newly generated instructions are intron"""
        res = False

        if len(treeStr.effRegisters) < ind.getNumRegs() and p2.getIndex() in treeStr.effRegisters:
            remain_set = set(range(0, ind.getNumRegs())) - treeStr.effRegisters
            p2.setIndex(state.random[thread].choice(list(remain_set)))
            if p1.printRootedTreeInString() == p2.printRootedTreeInString() \
                and len(remain_set) > 1:
                res = False
            else:
                res = True
